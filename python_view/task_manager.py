...
def load_dll():
    try:
        ...
        if getattr(sys, 'frozen', False):
            ...
            debug_print(f"Running as frozen application from {base_path}")
        else:
            base_path = os.path.dirname(os.path.abspath(__file__))
            debug_print(f"Running in development mode from {base_path}")
        
        dll_name = 'Dll2.dll'
        dll_paths = [
            # Try the absolute path first
            os.path.abspath(dll_name),
            # Then try current directory
            os.path.join(os.getcwd(), dll_name),
            # Then try the base path
            os.path.join(base_path, dll_name),
            # Then try parent directory
            os.path.join(os.path.dirname(base_path), dll_name),
            # Finally try python_view directory
            os.path.join(os.path.dirname(base_path), 'python_view', dll_name)
        ]
        
        debug_print("Searching for DLL in following locations:")
        for path in dll_paths:
            debug_print(f"- {path}")
            if os.path.exists(path):
                debug_print(f"Found DLL at: {path}")
                try:
                    # Try to load kernel32 first to get access to LoadLibrary
                    try:
                        kernel32 = ctypes.WinDLL('kernel32', use_last_error=True)
                        debug_print("Successfully loaded kernel32.dll")
                    except Exception as e:
                        debug_print(f"Failed to load kernel32.dll: {e}")
                        continue

                    # Set DLL directory to help Windows find dependencies
                    dll_directory = os.path.dirname(os.path.abspath(path))
                    debug_print(f"Setting DLL directory to: {dll_directory}")
                    try:
                        if kernel32.SetDllDirectoryW(dll_directory):
                            debug_print("Successfully set DLL directory")
                        else:
                            error_code = ctypes.get_last_error()
                            debug_print(f"Failed to set DLL directory, error code: {error_code}")
                    except Exception as e:
                        debug_print(f"Error setting DLL directory: {e}")

                    # Try to load the DLL using LoadLibrary first
                    try:
                        handle = kernel32.LoadLibraryW(path)
                        if handle:
                            debug_print("Successfully loaded DLL via LoadLibrary")
                            dll = ctypes.WinDLL(None, handle=handle)
                            # Test if we can access the GetProcessInfo function
                            try:
                                dll.GetProcessInfo
                                debug_print("Successfully accessed GetProcessInfo function")
                                return dll
                            except Exception as e:
                                debug_print(f"Failed to access GetProcessInfo function: {e}")
                                kernel32.FreeLibrary(handle)
                        else:
                            error_code = ctypes.get_last_error()
                            debug_print(f"LoadLibrary failed with error code: {error_code}")
                    except Exception as e:
                        debug_print(f"Error in LoadLibrary: {e}")

                    # Try direct loading as CDLL
                    try:
                        debug_print("Attempting to load as CDLL...")
                        dll = ctypes.CDLL(path)
                        # Test if we can access the GetProcessInfo function
                        dll.GetProcessInfo
                        debug_print("Successfully loaded as CDLL")
                        return dll
                    except Exception as e:
                        debug_print(f"Failed to load as CDLL: {e}")

                    # Try loading with explicit dependency loading
                    try:
                        debug_print("Attempting to load with explicit dependencies...")
                        # Load core Windows DLLs first
                        core_dlls = ['kernel32.dll', 'user32.dll', 'advapi32.dll']
                        for core_dll in core_dlls:
                            try:
                                ctypes.WinDLL(core_dll)
                                debug_print(f"Loaded core DLL: {core_dll}")
                            except Exception as e:
                                debug_print(f"Failed to load core DLL {core_dll}: {e}")

                        # Try loading the DLL again
                        dll = ctypes.WinDLL(path)
                        dll.GetProcessInfo  # Test access
                        debug_print("Successfully loaded DLL with explicit dependencies")
                        return dll
                    except Exception as e:
                        debug_print(f"Failed to load with explicit dependencies: {e}")

                except Exception as e:
                    debug_print(f"Error loading DLL at {path}: {e}")
                    debug_print(f"Error type: {type(e)}")
                    error_code = ctypes.get_last_error()
                    debug_print(f"Last error code: {error_code}")
                    continue

        debug_print("Could not load DLL from any location")
        return None

    except Exception as e:
        debug_print(f"Critical error in load_dll: {e}")
        debug_print(f"Error type: {type(e)}")
        return None
...
def run_as_admin():
    ...
    if sys.argv[-1] != 'asadmin':
        ...
        try:
            ...
            sys.exit(0)  # Выходим после запуска с правами администратора
        except Exception as e:
            debug_print(f"Ошибка при запуске с правами администратора: {e}")
            return False
    else:
        debug_print("Программа уже запущена с параметром asadmin")
        return True
...
class SystemMetrics:
    def __init__(self):
        debug_print("Initializing SystemMetrics...")
        ...
        # Try to load the DLL
        self.process_dll = load_dll()
        if self.process_dll:
            try:
                # Configure the GetProcessInfo function
                self.process_dll.GetProcessInfo.argtypes = [ctypes.c_ulong]
                self.process_dll.GetProcessInfo.restype = self.ProcessInfoStruct
                self.use_dll = True
                debug_print("GetProcessInfo function configured successfully")
            except Exception as e:
                debug_print(f"Failed to configure GetProcessInfo function: {e}")
                self.use_dll = False
        else:
            debug_print("Failed to load DLL, system metrics will be limited")
            self.use_dll = False
            
        # Initialize data caching
        self._process_data_cache = {}
        self._last_update_time = 0
        self._system_cpu_usage = 0.0
        self._system_memory = {"total": 0, "available": 0, "percent": 0}
        self._system_disk_io = {"read_bytes": 0.0, "write_bytes": 0.0}
        self._system_network_io = {"bytes_sent": 0.0, "bytes_recv": 0.0}
...
class UsersTab(QWidget):
    ...
    def get_process_username(self, pid):
        try:
            # Получаем имя пользователя процесса через WinAPI
            ...
            if handle:
                ...
                if ctypes.windll.advapi32.OpenProcessToken(handle, 0x0008, ctypes.byref(token)):
                    ...
                    user_sid_size = ctypes.c_ulong()
                    name_size = ctypes.c_ulong(260)
                    domain_size = ctypes.c_ulong(260)
                    name = ctypes.create_unicode_buffer(260)
                    domain = ctypes.create_unicode_buffer(260)
                    sid_type = ctypes.c_ulong()
                    
                    # Получаем SID пользователя
                    if ctypes.windll.advapi32.GetTokenInformation(token, 1, ctypes.byref(user_sid), ctypes.sizeof(user_sid), ctypes.byref(user_sid_size)):
                        # Получаем имя пользователя
                        if ctypes.windll.advapi32.LookupAccountSidW(None, user_sid, name, ctypes.byref(name_size), domain, ctypes.byref(domain_size), ctypes.byref(sid_type)):
                            username = name.value
                            ctypes.windll.advapi32.FreeSid(user_sid)
            ...
            debug_print(f"Error getting username for PID {pid}: {e}")
        
        # Если не удалось получить имя пользователя, возвращаем текущего пользователя
    ...
    def update_data(self, system_info):
        ...
        # Инициализируем статистику пользователей
        user_stats = {}
        ...
        for proc_info in system_info.get('processes', []):
            try:
                ...
                # Получаем имя пользователя для процесса
                username = self.get_process_username(pid)
                
                # Инициализируем статистику для пользователя, если её ещё нет
                ...
                user_stats[username]['cpu'] += proc_info.get('cpu_percent', 0)
                user_stats[username]['memory'] += proc_info.get('memory_info', {}).get('rss', 0)
        ...
        # Распределяем общие значения диска и сети между пользователями
        user_count = len(user_stats) if user_stats else 1
        disk_per_user = (total_disk_read + total_disk_write) / user_count
        net_per_user = (total_net_sent + total_net_recv) / user_count
        
        for username in user_stats:
            # Конвертируем память в МБ
            user_stats[username]['memory'] /= (1024 * 1024)
            
            # Добавляем общие значения диска и сети
            user_stats[username]['disk'] = disk_per_user
            user_stats[username]['network'] = net_per_user
